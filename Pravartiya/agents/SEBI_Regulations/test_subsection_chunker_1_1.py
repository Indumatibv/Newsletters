...
def create_subsection_chunks(chunks):
    """
    Takes a list of section-level chunks (as produced by the existing
    create_regulation_chunks) and returns a new list where each chunk is
    further split into subsection-level chunks wherever the regulation
    text contains valid, strictly-increasing subsection markers
    (e.g. (1), (2), (2A), (2B), (3)).
...
def process_chunks_file(input_path, output_path):
    with open(input_path, "r", encoding="utf-8") as f:
        data = json.load(f)

    if isinstance(data, dict) and "chunks" in data:
        data["chunks"] = create_subsection_chunks(data["chunks"])
        data["total_chunks"] = len(data["chunks"])
    elif isinstance(data, list):
        data = create_subsection_chunks(data)
    else:
...
if __name__ == "__main__":
    input_path = sys.argv[1] if len(sys.argv) > 1 else "old_chunks.json"
    output_path = sys.argv[2] if len(sys.argv) > 2 else "subsection_chunks.json"

    result = process_chunks_file(input_path, output_path)

    if isinstance(result, dict):
        print(f"Success! Created {result['total_chunks']} subsection-level chunks.")
    else:
        print(f"Success! Created {len(result)} subsection-level chunks.")