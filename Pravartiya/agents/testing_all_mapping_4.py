...
#     # 1. Load the filtered footers dataset
...
#         # Step A: Find all unique (Chapter, Section) combinations that reference this footer
#         matched_combinations = set()
#         for chunk in regulation_chunks:
#             if footer_id_int in chunk.get("footer_reference", []):
#                 ch_name = chunk.get("chapter")
#                 sec_no = chunk.get("section")
#                 if sec_no:
#                     # We store it as a tuple to lock the section to its specific chapter parent
#                     matched_combinations.add((ch_name, sec_no))

#         # Step B: Collect ALL chunks that belong to those exact combinations
#         matching_chunks = []
#         for chunk in regulation_chunks:
#             current_ch = chunk.get("chapter")
#             current_sec = chunk.get("section")
            
#             # Check if this chunk belongs to the exact chapter and section we caught in Step A
#             if (current_ch, current_sec) in matched_combinations:
#                 matched_chunk_copy = chunk.copy()
                
#                 # Optional cleanup: remove the raw reference array from the nested chunk
...
#         # 4. Map the payload under the footer number key
#         mapped_output[footer_num] = {
#             "footer_text": footer_text,
#             # For tracking clarity, lists out the exact chapter/section boundaries that were pulled
#             "matched_boundaries": [f"{ch} -> Section {sec}" for ch, sec in matched_combinations],
#             "mapped_regulation_chunks": matching_chunks
#         }

#     # 5. Write the precise combined dataset to a new JSON file
#     with open(output_json_path, "w", encoding="utf-8") as f:
#         json.dump(mapped_output, f, indent=4, ensure_ascii=False)

#     print(f"Mapping complete! Successfully tied {len(filtered_footers)} footers to exact chapter-section scopes.")
#     print(f"Saved precise mapping relationship file to '{output_json_path}'.")
